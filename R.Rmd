...
gridSearch <- function(df_train, df_prueba, maxdepth_hasta, minsplit_hasta, minbucket_hasta) {
    maxd = 1
    mins = 1
    minb = 1
    mejor_auc = 0
      ...
      for (j in 1:minsplit_hasta){
        for (k in 1:minbucket_hasta){
          tree <- rpart(formula = loan_status ~ person_age + person_gender + person_education + person_income + person_emp_exp + person_home_ownership + loan_amnt + loan_intent + loan_int_rate + loan_percent_income + cb_person_cred_hist_length + credit_score + previous_loan_defaults_on_file, 
                ...
                cp = 0,
                xval = 0)
          prediccion <- predict(tree, newdata = df_prueba, type = "class")
          auc <- AUC(prediccion, df_prueba$loan_status)
          if (auc > mejor_auc){
            maxd <- i
            mins <- j
            minb <- k
...
for (i in 1:20){
  ...
  AUCS_datos_20 <- append(AUCS_datos_20, matriz)
}
